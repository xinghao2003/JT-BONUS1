...
void __fastcall TForm2::btnEvaluateClick(TObject *Sender)
{
	...
	if (numbers.size() == 0) {
		btnRandomGen->OnClick(btnRandomGen);
	}
...
void powerThread(TRichEdit* rePowerOutput, int x, int n, int numIterations, bool usePow, bool useForLoop, bool useSquaring, bool useBitShifting) {
	// Clear the output at the beginning of the thread execution
	TThread::Synchronize(nullptr, [&]() {
		rePowerOutput->Clear();
	});
...
void __fastcall TForm2::btnPowerClick(TObject *Sender)
{
	...
	// Start the worker thread
	std::thread worker(powerThread, rePowerOutput, x, n, numIterations, usePow, useForLoop, useSquaring, useBitShifting);
	worker.detach(); // Detach to allow it to run independently
}
//---------------------------------------------------------------------------
